 ...
 *  @brief IAV polygon
 */
#define  MAX_POLYGON_VERTICE_NUM   (12)
struct iav_polygon {
	u8 vertice_num; //!< Number of vertices
	u8 reserved[3];
	struct iav_point vertex[MAX_POLYGON_VERTICE_NUM]; //!< Vertex coordinates
};
/* @} */
...
#define rect_contain(r1, r2) ({			\
	...
	_x1 <= _x2 && ((u64)_x1 + _w1) >= ((u64)_x2 + _w2) &&		\
	_y1 <= _y2 && ((u64)_y1 + _h1) >= ((u64)_y2 + _h2) ? 1 : 0;	\
	 })
/*! @} */

/*! @macros range_init(_x, _y)
 *  @brief struct @sa iav_range init macro
 */
#define range_init(_x, _y)(	\
{				\
	struct iav_range _r;		\
	_r.low = (_x); _r.high = (_y);		\
	_r;			\
})
...
{
	if (bit_map && nr >= 0 && nr < (int)(sizeof(u32) * 8)) {
		*bit_map |= (1U << nr);
	}
}
