...
#define IAV_FPS_DELTA	(5)

static inline u32 amba_iav_fps_format_to_vfr(u32 hp_fps, u32 format, u32 multi_frames)
{
	u32 vfr = AMBA_DSP_VFR_29_97_PROGRESSIVE;

	if ((hp_fps >= (IAV_HP_FPS_29_97 - IAV_FPS_DELTA)) &&
		(hp_fps <= (IAV_HP_FPS_29_97 + IAV_FPS_DELTA))) {
	...
	if ((hp_fps >= (IAV_HP_FPS_59_94 - IAV_FPS_DELTA)) &&
		(hp_fps <= (IAV_HP_FPS_59_94 + IAV_FPS_DELTA))) {
		if (format == AMBA_VIDEO_FORMAT_INTERLACE) {
	...
	if ((hp_fps >= (IAV_HP_FPS_23_976 - IAV_FPS_DELTA)) &&
		(hp_fps <= (IAV_HP_FPS_23_976 + IAV_FPS_DELTA))) {
		if (format == AMBA_VIDEO_FORMAT_INTERLACE) {
	...
	if ((hp_fps == IAV_HP_FPS_12_5) && (format == AMBA_VIDEO_FORMAT_INTERLACE)) {
		vfr = AMBA_DSP_VFR_12_5_INTERLACED;
		goto AMBA_IAV_FPS_FORMAT_TO_VFR_EXIT;
	}

	if ((hp_fps == IAV_HP_FPS_12_5) && (format == AMBA_VIDEO_FORMAT_PROGRESSIVE)) {
		vfr = AMBA_DSP_VFR_12_5_PROGRESSIVE;
		goto AMBA_IAV_FPS_FORMAT_TO_VFR_EXIT;
	}

	if ((hp_fps == IAV_HP_FPS_6_25) && (format == AMBA_VIDEO_FORMAT_INTERLACE)) {
		vfr = AMBA_DSP_VFR_6_25_INTERLACED;
		goto AMBA_IAV_FPS_FORMAT_TO_VFR_EXIT;
	}

	if ((hp_fps == IAV_HP_FPS_6_25) && (format == AMBA_VIDEO_FORMAT_PROGRESSIVE)) {
		vfr = AMBA_DSP_VFR_6_25_PROGRESSIVE;
		goto AMBA_IAV_FPS_FORMAT_TO_VFR_EXIT;
	}

	if ((hp_fps == IAV_HP_FPS_3_125) && (format == AMBA_VIDEO_FORMAT_INTERLACE)) {
		vfr = AMBA_DSP_VFR_3_125_INTERLACED;
		goto AMBA_IAV_FPS_FORMAT_TO_VFR_EXIT;
	}

	if ((hp_fps == IAV_HP_FPS_3_125) && (format == AMBA_VIDEO_FORMAT_PROGRESSIVE)) {
		vfr = AMBA_DSP_VFR_3_125_PROGRESSIVE;
		goto AMBA_IAV_FPS_FORMAT_TO_VFR_EXIT;
	}

	if ((hp_fps == IAV_HP_FPS_7_5) && (format == AMBA_VIDEO_FORMAT_INTERLACE)) {
		vfr = AMBA_DSP_VFR_7_5_INTERLACED;
		goto AMBA_IAV_FPS_FORMAT_TO_VFR_EXIT;
	}

	if ((hp_fps == IAV_HP_FPS_7_5) && (format == AMBA_VIDEO_FORMAT_PROGRESSIVE)) {
		vfr = AMBA_DSP_VFR_7_5_PROGRESSIVE;
		goto AMBA_IAV_FPS_FORMAT_TO_VFR_EXIT;
	}

	if ((hp_fps == IAV_HP_FPS_3_75) && (format == AMBA_VIDEO_FORMAT_INTERLACE)) {
		vfr = AMBA_DSP_VFR_3_75_INTERLACED;
		goto AMBA_IAV_FPS_FORMAT_TO_VFR_EXIT;
	}

	if ((hp_fps == IAV_HP_FPS_3_75) && (format == AMBA_VIDEO_FORMAT_PROGRESSIVE)) {
		vfr = AMBA_DSP_VFR_3_75_PROGRESSIVE;
		goto AMBA_IAV_FPS_FORMAT_TO_VFR_EXIT;
	}

	if ((hp_fps == IAV_HP_FPS_25) && (format == AMBA_VIDEO_FORMAT_PROGRESSIVE)) {
		vfr = AMBA_DSP_VFR_25_PROGRESSIVE;
		goto AMBA_IAV_FPS_FORMAT_TO_VFR_EXIT;
	}

	if ((hp_fps == IAV_HP_FPS_30) && (format == AMBA_VIDEO_FORMAT_PROGRESSIVE)) {
		vfr = AMBA_DSP_VFR_30_PROGRESSIVE;
		goto AMBA_IAV_FPS_FORMAT_TO_VFR_EXIT;
	}

	if ((hp_fps == IAV_HP_FPS_24) && (format == AMBA_VIDEO_FORMAT_PROGRESSIVE)) {
		vfr = AMBA_DSP_VFR_24_PROGRESSIVE;
		goto AMBA_IAV_FPS_FORMAT_TO_VFR_EXIT;
	}

	if ((hp_fps == IAV_HP_FPS_15) && (format == AMBA_VIDEO_FORMAT_PROGRESSIVE)) {
		vfr = AMBA_DSP_VFR_15_PROGRESSIVE;
		goto AMBA_IAV_FPS_FORMAT_TO_VFR_EXIT;
	}

	if ((hp_fps == IAV_HP_FPS_50) && (format == AMBA_VIDEO_FORMAT_PROGRESSIVE)) {
		vfr = AMBA_DSP_VFR_50_PROGRESSIVE;
		goto AMBA_IAV_FPS_FORMAT_TO_VFR_EXIT;
	}

	if ((hp_fps == IAV_HP_FPS_60) && (format == AMBA_VIDEO_FORMAT_PROGRESSIVE)) {
		vfr = AMBA_DSP_VFR_60_PROGRESSIVE;
		goto AMBA_IAV_FPS_FORMAT_TO_VFR_EXIT;
	}

	if ((hp_fps == IAV_HP_FPS_120) && (format == AMBA_VIDEO_FORMAT_PROGRESSIVE)) {
		vfr = AMBA_DSP_VFR_120_PROGRESSIVE;
		goto AMBA_IAV_FPS_FORMAT_TO_VFR_EXIT;
	}

	if (format == AMBA_VIDEO_FORMAT_INTERLACE) {
		vfr = DIV_ROUND(hp_fps, IAV_HP_FPS_FACTOR) | 0x80;
	} else if (format == AMBA_VIDEO_FORMAT_PROGRESSIVE) {
		vfr = DIV_ROUND(hp_fps, IAV_HP_FPS_FACTOR);
